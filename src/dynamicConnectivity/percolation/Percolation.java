 ...
 * http://algs4.cs.princeton.edu/code/javadoc/edu/princeton/cs/algs4/
 *         WeightedQuickUnionUF.html
 * 
...
public class Percolation {
    ...
    private void verifyIndex(int i, int j) {
        if ((i <= 0) || (i > n) || (j <= 0) || (j > n)) {
            throw new IndexOutOfBoundsException("Indices must be between 1 and " 
                    + n);
        }
    }
    
    /**
     * Transform external coordinates (row, column) to local index (between 1 
     * and n^2)
     * 
     * @param i - row index
     * @param j - column index
    ...
    private void connectLeftSite(int i, int j) {
        if (isOpen(i, j-1)) {
            wqu.union(transformToLocalIndex(i, j),
                    transformToLocalIndex(i, j-1));  
