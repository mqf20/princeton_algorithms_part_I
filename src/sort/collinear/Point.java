...
public class Point implements Comparable<Point> {
   ...
   * slopeTo() method.
   * 
   * Note that: - Points that are vertical to reference point have Double.POSITIVE_INFINITY slopes,
   * so they won't be sorted between vertically up or vertically down - Points that are horizontal
   * to reference point have +0.0 slopes, so they won't be sorted between horizontally left or
   * horizontally right - Degenerate (identical) points have Double.NEGATIVE_INFINITY slopes
   *
      ...
      public int compare(Point v, Point w) {
        ...
        if (vXDiff == 0) {
          if (vYDiff == 0) {
            ...
            // v is degenerate and w is not degenerate (may be vertical or horizontal or something
            // else)
            return -1;
          }
          if (wXDiff == 0) {
        ...
        return ((wXDiff * vYDiff) - (vXDiff * wYDiff)) * Integer.signum(wXDiff)
            * Integer.signum(vXDiff);
